...
func NewInvalidCredentials(msg ...string) *InvalidCredentials {
	defaultMsg := "group name already in use"
	if len(msg) == 0 {
...
// not group admin requestig access code
type PermissionForbidden struct {
	Err error
}

func (e *PermissionForbidden) Error() string {
	return fmt.Sprintf("%v", e.Err)
}

func NewPermissionForbidden(msg ...string) *PermissionForbidden {
	defaultMsg := "user requesting is not group admin"
	if len(msg) == 0 {
		return &PermissionForbidden{Err: errors.New(defaultMsg)}
	}
	return &PermissionForbidden{Err: errors.New(msg[0])}
}
...
func (e *InvalidAccessCode) Error() string {
	return fmt.Sprintf("%v", e.Err)
}

func NewInvalidAccessCode(msg ...string) *InvalidAccessCode {
	defaultMsg := "invalid access code"
	if len(msg) == 0 {
		return &InvalidAccessCode{Err: errors.New(defaultMsg)}
	}
	return &InvalidAccessCode{Err: errors.New(msg[0])}
}
